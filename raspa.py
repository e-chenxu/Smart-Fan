...
GPIO.setup(FAN, GPIO.OUT)                      # output of fan
fan = GPIO.PWM(FAN,100)                        # 100 Hz PWM frequency
fan.start(0)                                   # fan off
...
scan_range_right = 345   # max right scan
scan_range_left = 210    # max left scan
...
# fan speech change function

def change_speed(x, y, w, h):

    # high speed due to subject being further away
    if w < 50:
...
def loop():
    ...
    while True:
        ...
        if CFace != 0:
            track_face(CFace)
            change_speed(curr_x, curr_y, curr_w, curr_h)
            count = 0
            ...
            # turn off fan if hasnt seen a face in a while
            if count == 1000:
